...
files.forEach(file => {
  ...
  // Remove any duplicate or broken hamburger blocks
  content = content.replace(/\/\*\s*───\s*HAMBURGER MENU TOGGLE[\s\S]*?\)\(\);/gi, '');
  content = content.replace(/\/\*\s*───\s*HAMBURGER MENU[\s\S]*?(?=\/\*|<\/script>)/gi, '');
  content = content.replace(/const hamburger = document\.getElementById\('hamburger'\);[\s\S]*?\}\);\s*\}\);/gi, '');
